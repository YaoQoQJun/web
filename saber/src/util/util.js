...
/**
 * 截取最后一个字符后面的内容。一般用于取文件名
 */
export const getLastStrBySeparator = (str, separator) => {
  if (!validatenull(str)) {
    let index = str.lastIndexOf(separator);
    return str.substring(index + 1, str.length).toLowerCase();
  } else {
    return "";
  }
};
 ...
 * 文件上传前
 */
export const uploadBeforeCallback = (
  file,
  loading,
  column,
  suffixArr,
  ...
  if (suffixArr.includes(suffix) || suffixArr.length == 0) {
    if (suffixArr.includes("png")) {
      var freader = new FileReader();
      freader.readAsDataURL(file);
      freader.onload = function(e) {
        that.$nextTick(() => {
          that.form[prop].push(e.target.result);
        });
      };
    } else {
    ...
    if (fileUrlArr.length == 0) that.form[prop] = [];
    that.$alert(`请选择后缀名为${suffixArr.join(",")}的文件`);
  }
  loading();
};
 ...
 * 删除文件前
 */
export const uploadAfterCallback = (column, that) => {
  let prop = column.prop;
  that.file[prop] = "";
  that.form[prop] = [];
};
...
//驼峰转下划线
export const camecaseToLine = name => {
